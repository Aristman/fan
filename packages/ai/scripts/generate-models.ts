...
async function generateModels() {
	...
	// Qwen Token Plan models (Aliyun Singapore, OpenAI-compatible)
	// https://www.alibabacloud.com/help/en/model-studio/developer-reference/token-plan
	const QWEN_BASE_URL = "https://token-plan.ap-southeast-1.maas.aliyuncs.com/compatible-mode/v1";
	const qwenModels: Model<"openai-completions">[] = [
		{
			id: "deepseek-v3.2",
			name: "DeepSeek V3.2",
			api: "openai-completions",
	...
	// Defensive: only add if not already present from models.dev
	for (const model of qwenModels) {
		if (!allModels.some(m => m.provider === "qwen" && m.id === model.id)) {
			allModels.push(model);
		}
	}
