...
```bash
npm install @seaagents/fan-ai
```

TypeBox exports are re-exported from `@seaagents/fan-ai`: `Type`, `Static`, and `TSchema`.

## Quick Start

```typescript
import { Type, getModel, stream, complete, Context, Tool, StringEnum } from '@seaagents/fan-ai';
...
```typescript
import { Type, Tool, StringEnum } from '@seaagents/fan-ai';
...
```typescript
import { stream, validateToolCall, Tool } from '@seaagents/fan-ai';
...
```typescript
import { readFileSync } from 'fs';
import { getModel, complete } from '@seaagents/fan-ai';
...
```typescript
import { getModel, streamSimple, completeSimple } from '@seaagents/fan-ai';
...
```typescript
import { getModel, stream } from '@seaagents/fan-ai';
...
```typescript
import { getProviders, getModels, getModel } from '@seaagents/fan-ai';
...
```typescript
import { Model, stream } from '@seaagents/fan-ai';
...
```typescript
import { streamAnthropic, type AnthropicOptions } from '@seaagents/fan-ai';
...
```typescript
import { getModel, complete, Context } from '@seaagents/fan-ai';
...
```typescript
import { Context, getModel, complete } from '@seaagents/fan-ai';
...
- OAuth login flows are not supported in browser environments. Use the `@seaagents/fan-ai/oauth` entry point in Node.js.
...
```typescript
import { getEnvApiKey } from '@seaagents/fan-ai';
...
```bash
npx @seaagents/fan-ai login              # interactive provider selection
npx @seaagents/fan-ai login anthropic    # login to specific provider
npx @seaagents/fan-ai list               # list available providers
```
...
The library provides login and token refresh functions via the `@seaagents/fan-ai/oauth` entry point. Credential storage is the caller's responsibility.
...
} from '@seaagents/fan-ai/oauth';
```

### Login Flow Example

```typescript
import { loginGitHubCopilot } from '@seaagents/fan-ai/oauth';
...
```typescript
import { getModel, complete } from '@seaagents/fan-ai';
import { getOAuthApiKey } from '@seaagents/fan-ai/oauth';
...
- Add the API identifier to `KnownApi`
...
Create a new provider file that exports:
...
- Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@seaagents/fan-ai`
...
For providers with non-standard auth (Google Vertex), create a utility with credential detection helpers.
