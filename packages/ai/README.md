...
```bash
npm install @itone/fan-ai
```

TypeBox exports are re-exported from `@itone/fan-ai`: `Type`, `Static`, and `TSchema`.

## Quick Start

```typescript
import { Type, getModel, stream, complete, Context, Tool, StringEnum } from '@itone/fan-ai';
...
```typescript
import { Type, Tool, StringEnum } from '@itone/fan-ai';
...
```typescript
import { stream, validateToolCall, Tool } from '@itone/fan-ai';
...
```typescript
import { readFileSync } from 'fs';
import { getModel, complete } from '@itone/fan-ai';
...
```typescript
import { getModel, streamSimple, completeSimple } from '@itone/fan-ai';
...
```typescript
import { getModel, stream } from '@itone/fan-ai';
...
```typescript
import { getProviders, getModels, getModel } from '@itone/fan-ai';
...
```typescript
import { Model, stream } from '@itone/fan-ai';
...
```typescript
import { streamAnthropic, type AnthropicOptions } from '@itone/fan-ai';
...
```typescript
import { getModel, complete, Context } from '@itone/fan-ai';
...
```typescript
import { Context, getModel, complete } from '@itone/fan-ai';
...
- OAuth login flows are not supported in browser environments. Use the `@itone/fan-ai/oauth` entry point in Node.js.
...
```typescript
import { getEnvApiKey } from '@itone/fan-ai';
...
```bash
npx @itone/fan-ai login              # interactive provider selection
npx @itone/fan-ai login anthropic    # login to specific provider
npx @itone/fan-ai list               # list available providers
```
...
The library provides login and token refresh functions via the `@itone/fan-ai/oauth` entry point. Credential storage is the caller's responsibility.
...
} from '@itone/fan-ai/oauth';
```

### Login Flow Example

```typescript
import { loginGitHubCopilot } from '@itone/fan-ai/oauth';
...
```typescript
import { getModel, complete } from '@itone/fan-ai';
import { getOAuthApiKey } from '@itone/fan-ai/oauth';
...
- Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@itone/fan-ai`
