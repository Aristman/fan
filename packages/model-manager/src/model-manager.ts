...
export class ModelManager {
   ...
   */
  async trackUsage(provider: string, tokens: number, cost: number, period: string = "daily"): Promise<{ status: BudgetStatus; alert?: BudgetAlert }> {
  ...
  async getRoutingRules(): Promise<RoutingRuleData[]> {
    ...
    try { await this.router["ensureLoaded"](); } catch { /* ignore */ }
   ...
   */
  applySettingsToOptions<T extends Record<string, any>>(
  ...
  async syncFromSettings(settings: {
    ...
    budget?: { dailyTokenLimit?: number; dailyCostLimit?: number; monthlyTokenLimit?: number; monthlyCostLimit?: number };
  }): Promise<void> {
    ...
    if (settings.routingRules) {
      for (const rule of settings.routingRules) {
        ops.push(
          db.upsertRoutingRule(rule).then(() => {}),
        );
      }
    }
