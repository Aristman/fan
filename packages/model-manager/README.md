...
**Где используется:** `@fan/api-gateway` использует его для обработки API-запросов к моделям. `@itone/fan-coding-agent` использует его для выбора модели при выполнении задач. CLI-команды (`fan init`, `fan server`) полагаются на него для конфигурации моделей.
...
- `@itone/fan-ai` — типы и интерфейсы моделей ИИ
