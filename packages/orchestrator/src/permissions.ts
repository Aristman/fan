...
export function isDangerousCommand(cmd: string): string | null {
    ...
    if (/\brm\s+.*(?:-[a-zA-Z]*r[a-zA-Z]*|--recursive).*\s+.*(?:-[a-zA-Z]*f[a-zA-Z]*|--force)/i.test(stripped) ||
    ...
    if (/\brm\s+.*-[a-zA-Z]*rf[a-zA-Z]*/i.test(stripped) ||
        /\brm\s+.*-[a-zA-Z]*fr[a-zA-Z]*/i.test(stripped)) {
        return "Recursive forced delete (rm -rf)";
    }
    ...
    if (/\b(?:DROP|TRUNCATE)\s+(?:TABLE|DATABASE|SCHEMA)\b/i.test(stripped) ||
