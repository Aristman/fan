...
export class TUI extends Container {
	...
	private showHardwareCursor = process.env.FAN_HARDWARE_CURSOR === "1";
	private clearOnShrink = process.env.FAN_CLEAR_ON_SHRINK === "1"; // Clear empty rows when content shrinks (default: off)
	...
	private doRender(): void {
		...
		const debugRedraw = process.env.FAN_DEBUG_REDRAW === "1";
		const logRedraw = (reason: string): void => {
			if (!debugRedraw) return;
			const logPath = path.join(os.homedir(), ".fan", "agent", "fan-debug.log");
		...
		// Configurable via setClearOnShrink() or FAN_CLEAR_ON_SHRINK=0 env var
		...
		for (let i = firstChanged; i <= renderEnd; i++) {
			...
			if (!isImage && visibleWidth(line) > width) {
				...
				const crashLogPath = path.join(os.homedir(), ".fan", "agent", "fan-crash.log");
				const crashData = [
		...
		if (process.env.FAN_TUI_DEBUG === "1") {
