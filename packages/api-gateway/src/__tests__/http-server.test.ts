...
describe("HTTP Server", () => {
  ...
  describe("Sessions", () => {
    ...
    it("POST /api/sessions should create a session", async () => {
      mockSessionAdapter.createSession.mockResolvedValueOnce({
        id: "s2", title: "New Session", createdAt: "2026-01-01",
      });
      const app = getApp();
      const res = await app.request("/api/sessions", {
        method: "POST",
    ...
    it("GET /api/sessions/:id should return session when found", async () => {
      mockSessionAdapter.getSession.mockResolvedValueOnce({
        id: "s1", title: "Found", createdAt: "2026-01-01", updatedAt: "2026-01-01",
        messages: [],
      });
      const app = getApp();
  ...
  describe("Messages", () => {
    ...
    it("POST /api/sessions/:id/messages should pass streamingBehavior", async () => {
      ...
      expect(mockSessionAdapter.sendMessage).toHaveBeenCalledWith(
        "s1", "Hello", "steer",
      );
    });
  });
  ...
  describe("Models", () => {
    it("GET /api/models should return models and routing rules", async () => {
      ...
      mockModelManager.getRoutingRules.mockResolvedValueOnce([
        { id: "r1", name: "coding", provider: "anthropic", model: "claude-sonnet", fallback: undefined, enabled: true },
      ]);
      const app = getApp();
    ...
    it("GET /api/models/settings should return settings", async () => {
      mockModelManager.getAllModelSettings.mockResolvedValueOnce([
        { id: "ms1", provider: "anthropic", model: "claude-sonnet", temperature: 0.7, maxTokens: null, thinking: null, isDefault: false, priority: 0 },
      ]);
      const app = getApp();
    ...
    it("PUT /api/models/settings should update setting", async () => {
      const updatedSetting = { id: "ms1", provider: "anthropic", model: "claude-sonnet", temperature: 0.5, maxTokens: null, thinking: null, isDefault: false, priority: 0 };
  ...
  describe("Budget", () => {
    ...
    it("GET /api/budget should wrap non-array budgets in array", async () => {
      mockModelManager.getBudgetStatus.mockResolvedValueOnce(
        { provider: "anthropic", period: "daily", tokensUsed: 1000, costUsed: 0.05, exceeded: false } as any,
      );
      const app = getApp();
      const res = await app.request("/api/budget");
      expect(res.status).toBe(200);
  ...
  describe("Tokens", () => {
    it("POST /api/tokens should generate a token", async () => {
      const mockToken = {
        id: "t1", name: "Test", token: "hex-token",
        createdAt: new Date("2026-01-01"), lastUsed: null,
      };
