...
export async function listTokens(): Promise<Omit<ClientTokenData, "token">[]> {
  ...
  return records.map(({ token: _token, ...rest }: { id: string; name: string; token: string; createdAt: Date; lastUsed: Date | null }) => rest);
}
...
function mapToClientToken(record: { id: string; name: string; token: string; createdAt: Date; lastUsed: Date | null }): ClientTokenData {
  return {
    id: record.id,
    name: record.name,
    token: record.token,
