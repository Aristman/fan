...
import { validateToken, isAuthDisabled } from "./auth.js";
import type { SessionAdapter } from "./http-server.js";
import type { WsOutgoingMessage, WsIncomingMessage } from "./types.js";
      ...
      wss.on("connection", (ws: WsWebSocket, request: IncomingMessage) => {
        ...
        ws.on("message", (data: Buffer) => {
          try {
            ...
            if (msg.type === "ping") {
              ws.send(JSON.stringify({ type: "pong", sessionId: connSessionId, timestamp: new Date().toISOString() }));
            }
