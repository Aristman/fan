...
import { backoffDelay, createMcpClientManager } from "../../src/manager.js";
...
afterAll(async () => {
	try {
		await unlink(join(tempDir, "test-e2e-read.txt")).catch(() => {});
	} catch {
		/* ok */
	}
});

function mkdtempSync(_prefix: string): string {
...
describe("Test 1: Coordinator MCP tool call (F-1.6 + F-1.7)", () => {
	it("connects to realistic-server, calls echo tool, gets real response", { timeout: 15_000 }, async () => {
		...
		if (entries[0].status === "unavailable") {
			console.warn("Realistic server unavailable, skipping");
			await mgr.dispose();
			return;
		}
	...
	it("calls read_file on temp file via real server", { timeout: 15_000 }, async () => {
		...
		if (entries[0].adapterClient) {
			const result = await entries[0].adapterClient.callTool({
				name: "read_file",
...
describe("Test 2: Permission gate blocking (F-1.10 + BUG-1, BUG-2)", () => {
	it("blocks tool matching deniedTools", () => {
		const gate = createPermissionGate([
			{ transport: "stdio" as const, command: "x", allowedTools: ["*"], deniedTools: ["delete_file"] },
		]);
		expect(gate.gate({ toolName: "mcp__0__delete_file" } as any).block).toBe(true);
	});
	it("allows tool in allowlist", () => {
		const gate = createPermissionGate([
			{ transport: "stdio" as const, command: "x", allowedTools: ["read_file", "echo"], deniedTools: [] },
		]);
		expect(gate.gate({ toolName: "mcp__0__read_file" } as any).block).toBeUndefined();
	});
	it("blocks tool NOT in allowlist", () => {
		const gate = createPermissionGate([
			{ transport: "stdio" as const, command: "x", allowedTools: ["read_*"], deniedTools: [] },
		]);
...
describe("Test 3: Auto-restart after crash (F-1.17 + F-3.4)", () => {
	it("trigger onclose, tools removed", { timeout: 15_000 }, async () => {
		...
		await mgr.connectAll({
			servers: [{ transport: "stdio", command: "node", args: [fixturePath], autoRestart: false }],
		});
		const entries = mgr._entries();
		if (entries.length === 0 || entries[0].status === "unavailable") {
			console.warn("stdio-server not connectable");
			await mgr.dispose();
			return;
		}
		expect(entries[0].status).toBe("connected");
	...
	it("auto-restart backoff triggers reconnect", { timeout: 30_000 }, async () => {
		...
		const mgr = createMcpClientManager(api, gate);
		await mgr.connectAll({
			servers: [{ transport: "stdio", command: "node", args: [fixturePath], autoRestart: true, timeout: 5_000 }],
		});
		const entries = mgr._entries();
		if (entries.length === 0 || entries[0].status === "unavailable") {
			console.warn("stdio-server not connectable");
...
describe("Test 4: list_changed atomic refresh (F-1.15)", () => {
	it("notify-server: tools updated after list_changed", { timeout: 15_000 }, async () => {
		...
		const mgr = createMcpClientManager(api, gate);
		await mgr.connectAll({
			servers: [{ transport: "stdio", command: "node", args: [fixturePath], timeout: 10_000 }],
		});
		const entries = mgr._entries();
		if (entries.length === 0 || entries[0].status !== "connected") {
			console.warn("notify-server not connectable");
			await mgr.dispose();
			return;
		}
	...
	it("diff-based refresh: add/remove/update", () => {
		...
		expect(
			["mcp__0__greet", "mcp__0__math_add"].filter((n) => !new Set(["mcp__0__greet", "mcp__0__weather"]).has(n)),
		).toEqual(["mcp__0__math_add"]);
	});
});
...
describe("Test 5: Worker proxy - mock stdin/stdout (F-2.6 + F-2.7)", () => {
	it("remote_tool_request/response JSONL protocol roundtrip", () => {
		...
		const catalog = deserialize(
			serialize({ type: "remote_tool_catalog", tools: [{ id: "mcp__0__echo", name: "echo" }] }),
		);
		expect(catalog.type).toBe("remote_tool_catalog");
		expect(catalog.tools[0].id).toBe("mcp__0__echo");
		const req = deserialize(
			serialize({ type: "remote_tool_request", id: "inv-001", toolId: "mcp__0__echo", args: { text: "hello" } }),
		);
			...
			return childOutput.map((line) => {
				...
				if (msg.type === "remote_tool_request") {
					return `${JSON.stringify({
						type: "remote_tool_response",
						id: msg.id,
						content: [{ type: "text" as const, text: msg.args.text }],
						isError: false,
					})}\n`;
				}
				return "";
...
describe("Test 6: Profile filtering (F-2.7)", () => {
	it("filterToolsByConfig: read-only profile", () => {
		const tools = [
			{ name: "mcp__0__read_file" },
			{ name: "mcp__0__write_file" },
			{ name: "mcp__0__list_dir" },
			{ name: "mcp__0__echo" },
		];
		...
		const filtered = filterToolsByConfig(tools, {
			allowedTools: ["mcp__0__read_file", "mcp__0__list_dir", "mcp__0__echo"],
		});
		expect(filtered).toHaveLength(3);
	});
	it("filterToolsByConfig: all profile", () => {
		expect(
			filterToolsByConfig([{ name: "mcp__0__read_file" }, { name: "mcp__0__write_file" }], { allowedTools: ["*"] }),
		).toHaveLength(2);
	});
	...
	it("deniedTools removes matched tools", () => {
		...
		const result = filterToolsByConfig([{ name: "mcp__0__read_file" }, { name: "mcp__0__write_file" }], {
			allowedTools: ["*"],
...
describe("Test 7: /mcp status command (F-3.5)", () => {
	it("mcpExtension registers the /mcp command", async () => {
		...
		expect(api.registerCommand).toHaveBeenCalledWith(
			"mcp",
			expect.objectContaining({ description: expect.stringContaining("MCP") }),
		);
	});
...
describe("Test 9: Settings load + merge (F-1.8 + F-1.18)", () => {
	it("project overrides global by serverId index", async () => {
		...
		await writeFile(
			...
			JSON.stringify({ servers: [{ transport: "stdio", command: "global-cmd", timeout: 5000 }] }),
			"utf8",
		);
		await writeFile(
			join(dir, "project.json"),
			JSON.stringify({ servers: [{ transport: "stdio", command: "project-cmd" }] }),
			"utf8",
		);
	...
	it("invalid JSON in project => global still loads", async () => {
		...
		try {
			...
			await writeFile(
				join(dir, "global.json"),
				JSON.stringify({ servers: [{ transport: "stdio", command: "ok" }] }),
				"utf8",
			);
	...
	it("project has more servers => extra appended", async () => {
		...
		await writeFile(
			join(dir, "global.json"),
			JSON.stringify({ servers: [{ transport: "stdio", command: "a" }] }),
			"utf8",
		);
		await writeFile(
			join(dir, "project.json"),
...
describe("Test 10: Logging (F-3.7 + BUG-9)", () => {
	it("withLogging redacts Bearer tokens in errorMessage", async () => {
		...
		await expect(
			withLogging(
				...
				"read_file",
				async () => {
					throw new Error("Authorization failed: Bearer abc123def456");
				},
				tempLogDir,
			),
		).rejects.toThrow();
	...
	it("withLogging redacts sk- secrets", async () => {
		...
		await expect(
			withLogging(
				...
				"query",
				async () => {
					throw new Error("sk-0123456789abcdef0123456789abcdef");
				},
				tempLogDir,
			),
		).rejects.toThrow();
	...
	it("writeLog creates directory and appends entries", async () => {
		const tempLogDir = await createTempDir();
		await writeLog(
			{ timestamp: new Date().toISOString(), event: "tool_call_start", serverId: "fs", toolName: "read" },
			tempLogDir,
		);
		await writeLog(
			{
