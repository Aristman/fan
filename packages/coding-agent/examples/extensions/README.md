...
Example extensions for fan-coding-agent.
...
cp permission-gate.ts ~/.fan/agent/extensions/
```
...
| `send-user-message.ts` | Demonstrates `fan.sendUserMessage()` for sending user messages from extensions |
...
| `event-bus.ts` | Inter-extension communication via `fan.events` |
