...
export default function planModeExtension(fan: ExtensionAPI): void {
	...
	function togglePlanMode(ctx: ExtensionContext): void {
		...
		if (planModeEnabled) {
			fan.setActiveTools(PLAN_MODE_TOOLS);
			ctx.ui.notify(`Plan mode enabled. Tools: ${PLAN_MODE_TOOLS.join(", ")}`);
		} else {
			fan.setActiveTools(NORMAL_MODE_TOOLS);
	...
	fan.registerShortcut(Key.ctrlAlt("p"), {
...
After completing a step, include a [DONE:n] tag in your response.`,
	...
	fan.on("agent_end", async (event, ctx) => {
		...
		if (choice?.startsWith("Execute")) {
			...
			if (refinement?.trim()) {
				fan.sendUserMessage(refinement.trim());
			}
		}
	});
