...
import type { ExtensionAPI, SessionBeforeSwitchEvent, SessionMessageEntry } from "@seaagents/fan-coding-agent";

export default function (fan: ExtensionAPI) {
	fan.on("session_before_switch", async (event: SessionBeforeSwitchEvent, ctx) => {
		if (!ctx.hasUI) return;
