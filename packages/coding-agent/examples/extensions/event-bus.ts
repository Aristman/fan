 ...
 *
 * Shows fan.events for communication between extensions. One extension
...
export default function (fan: ExtensionAPI) {
	...
	fan.events.on("my:notification", (data) => {
	...
	fan.on("session_start", async () => {
		fan.events.emit("my:notification", {
