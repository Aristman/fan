 ...
 *
 * Demonstrates fan.sendUserMessage() for sending user messages from extensions.
 * Unlike fan.sendMessage() which sends custom messages, sendUserMessage() sends
...
export default function (fan: ExtensionAPI) {
	...
	fan.registerCommand("ask", {
		...
		handler: async (args, ctx) => {
			...
			fan.sendUserMessage(args);
		},
	});

	// Command that steers the agent mid-conversation
	fan.registerCommand("steer", {
		...
		handler: async (args, ctx) => {
			...
			if (ctx.isIdle()) {
				...
				fan.sendUserMessage(args, { deliverAs: "steer" });
			}
		},
	});

	// Command that queues a follow-up message
	fan.registerCommand("followup", {
		...
		handler: async (args, ctx) => {
			...
			if (ctx.isIdle()) {
				...
				fan.sendUserMessage(args, { deliverAs: "followUp" });
