...
export default function (fan: ExtensionAPI) {
	...
	fan.on("session_start", async (event, ctx) => {
		ctx.ui.setTitle(event.reason === "new" ? "fan RPC Demo (new session)" : "fan RPC Demo");
	...
	fan.on("turn_start", async (_event, ctx) => {
		turnCount++;
	...
	fan.registerCommand("rpc-editor", {
	...
	fan.registerCommand("rpc-prefill", {
