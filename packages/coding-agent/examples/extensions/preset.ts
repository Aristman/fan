 ...
 * - ~/.fan/agent/presets.json (global)
 * - <cwd>/.fan/presets.json (project-local)
 *
...
function loadPresets(cwd: string): PresetsConfig {
	...
	const projectPath = join(cwd, ".fan", "presets.json");
...
export default function presetExtension(fan: ExtensionAPI) {
	let presets: PresetsConfig = {};
	let activePresetName: string | undefined;
	let activePreset: Preset | undefined;

	// Register --preset CLI flag
	fan.registerFlag("preset", {
	...
	async function applyPreset(name: string, preset: Preset, ctx: ExtensionContext): Promise<boolean> {
		...
		if (preset.thinkingLevel) {
			fan.setThinkingLevel(preset.thinkingLevel);
		}

		// Apply tools if specified
		if (preset.tools && preset.tools.length > 0) {
			const allToolNames = fan.getAllTools().map((t) => t.name);
	...
	async function showPresetSelector(ctx: ExtensionContext): Promise<void> {
		...
		if (presetNames.length === 0) {
			ctx.ui.notify("No presets defined. Add presets to ~/.fan/agent/presets.json or .fan/presets.json", "warning");
			return;
		}
		...
		if (result === "(none)") {
			...
			fan.setActiveTools(["read", "bash", "edit", "write"]);
	...
	fan.registerShortcut(Key.ctrlShift("u"), {
	...
	fan.on("session_start", async (_event, ctx) => {
		...
		const presetFlag = fan.getFlag("preset");
	...
	fan.on("turn_start", async () => {
		if (activePresetName) {
			fan.appendEntry("preset-state", { name: activePresetName });
		}
	});
}
