 ...
 * Commands Extension
 *
 * Demonstrates the fan.getCommands() API by providing a /commands command
 * that lists all available slash commands in the current session.
 *
 * Usage:
 * 1. Copy this file to ~/.fan/agent/extensions/ or your project's .fan/extensions/
 * 2. Use /commands to see available commands
 * 3. Use /commands extensions to filter by source
 */

import type { ExtensionAPI, SlashCommandInfo } from "@seaagents/fan-coding-agent";

export default function commandsExtension(fan: ExtensionAPI) {
	fan.registerCommand("commands", {
