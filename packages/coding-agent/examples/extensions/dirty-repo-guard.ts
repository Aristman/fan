...
async function checkDirtyRepo(
	...
	const { stdout, code } = await fan.exec("git", ["status", "--porcelain"]);
...
export default function (fan: ExtensionAPI) {
	fan.on("session_before_switch", async (event, ctx) => {
		...
		return checkDirtyRepo(fan, ctx, action);
	});

	fan.on("session_before_fork", async (_event, ctx) => {
		return checkDirtyRepo(fan, ctx, "fork");
	});
}
