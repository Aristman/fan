...
export default function (fan: ExtensionAPI) {
	fan.registerFlag("ssh", { description: "SSH remote: user@host or user@host:/path", type: "string" });
	...
	fan.on("session_start", async (_event, ctx) => {
		...
		const arg = fan.getFlag("ssh") as string | undefined;
		if (arg) {
			if (arg.includes(":")) {
