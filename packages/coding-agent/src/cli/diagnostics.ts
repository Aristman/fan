...
// Central diagnostics module for "fna doctor" command

import { getAgentDir, getModelsPath, getSettingsPath, getSessionsDir, VERSION, APP_NAME } from "../config.js";
import { mkdirSync, accessSync, constants, readFileSync, existsSync, readdirSync } from "node:fs";
...
export async function runDiagnostics(): Promise<boolean> {
  ...
  results.push({
    name: "Runtime mode",
    status: "ok",
    message: process.argv[0].endsWith("node") || process.argv[0].endsWith("node.exe")
      ? "Node.js module"
      : "Standalone binary",
  });
  ...
  if (existsSync(settingsPath)) {
    ...
    results.push({
      ...
      detail: "Run 'fna init' to create one",
    });
  }
