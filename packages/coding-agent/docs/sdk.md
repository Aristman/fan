> fan can help you use the SDK. Ask it to build an integration for your use case.

# SDK

The SDK provides programmatic access to fan's agent capabilities. Use it to embed fan in other applications, build custom interfaces, or integrate with automated workflows.
...
**Behavior:**
- **Extension commands** (e.g., `/mycommand`): Execute immediately, even during streaming. They manage their own LLM interaction via `fan.sendMessage()`.
...
The `Agent` class (from `@seaagents/fan-agent-core`) handles the core LLM interaction. Access it via `session.agent`.
...
const { session } = await createAgentSession({
  ...
  agentDir: "~/.fan/agent", // default (expands ~)
});
```

`cwd` is used by `DefaultResourceLoader` for:
- Project extensions (`.fan/extensions/`)
- Project skills:
  - `.fan/skills/`
  - `.agents/skills/` in `cwd` and ancestor directories (up to git repo root, or filesystem root when not in a repo)
- Project prompts (`.fan/prompts/`)
...
- Global skills:
  - `skills/` under `agentDir` (for example `~/.fan/agent/skills/`)
  - `~/.agents/skills/`
...
import { AuthStorage, ModelRegistry } from "@seaagents/fan-coding-agent";
...
// Default: uses ~/.fan/agent/auth.json and ~/.fan/agent/models.json
const authStorage = AuthStorage.create();
const modelRegistry = ModelRegistry.create(authStorage);
...
```typescript
import { createAgentSession, DefaultResourceLoader } from "@seaagents/fan-coding-agent";
...
- If you omit `tools`, fan automatically creates them with the correct `cwd`
...
import { createAgentSession, defineTool } from "@seaagents/fan-coding-agent";
...
Use `defineTool()` for standalone definitions and arrays like `customTools: [myTool]`. Inline `fan.registerTool({ ... })` already infers parameter types correctly.

Custom tools passed via `customTools` are combined with extension-registered tools. Extensions loaded by the ResourceLoader can also register tools via `fan.registerTool()`.

> See [examples/sdk/05-tools.ts](../examples/sdk/05-tools.ts)

### Extensions

Extensions are loaded by the `ResourceLoader`. `DefaultResourceLoader` discovers extensions from `~/.fan/agent/extensions/`, `.fan/extensions/`, and settings.json extension sources.
...
**Event Bus:** Extensions can communicate via `fan.events`. Pass a shared `eventBus` to `DefaultResourceLoader` if you need to emit or listen from outside:

```typescript
import { createEventBus, DefaultResourceLoader } from "@seaagents/fan-coding-agent";
...
```typescript
import { createAgentSession, SettingsManager, SessionManager } from "@seaagents/fan-coding-agent";
...
1. Global: `~/.fan/agent/settings.json`
2. Project: `<cwd>/.fan/settings.json`
