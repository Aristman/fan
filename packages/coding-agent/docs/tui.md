> fan can create TUI components. Ask it to build one for your use case.

# TUI Components

Extensions and custom tools can render custom TUI components for interactive user interfaces. This page covers the component system and available building blocks.

**Source:** [`@seaagents/fan-tui`](https://github.com/seaagents/fan/tree/main/packages/tui)
...
```typescript
import { CURSOR_MARKER, type Component, type Focusable } from "@seaagents/fan-tui";
...
```typescript
import { Container, type Focusable, Input } from "@seaagents/fan-tui";
...
**In custom tools** via `fan.ui.custom()`:

```typescript
async execute(toolCallId, params, onUpdate, ctx, signal) {
  const handle = fan.ui.custom(myComponent);
  // ...
  handle.close();
}
```
...
Import from `@seaagents/fan-tui`:

```typescript
import { Text, Box, Container, Spacer, Markdown } from "@seaagents/fan-tui";
```
...
```typescript
import { matchesKey, Key } from "@seaagents/fan-tui";
...
```typescript
import { visibleWidth, truncateToWidth } from "@seaagents/fan-tui";
...
import { Markdown } from "@seaagents/fan-tui";
...
Set `FAN_TUI_WRITE_LOG` to capture the raw ANSI stream written to stdout.

```bash
FAN_TUI_WRITE_LOG=/tmp/tui-ansi.log npx tsx packages/tui/test/chat-simple.ts
```
...
For letting users pick from a list of options. Use `SelectList` from `@seaagents/fan-tui` with `DynamicBorder` for framing.

```typescript
import type { ExtensionAPI } from "@seaagents/fan-coding-agent";
import { DynamicBorder } from "@seaagents/fan-coding-agent";
import { Container, type SelectItem, SelectList, Text } from "@seaagents/fan-tui";
...
```typescript
import { BorderedLoader } from "@seaagents/fan-coding-agent";

fan.registerCommand("fetch", {
  handler: async (_args, ctx) => {
    const result = await ctx.ui.custom<string | null>((tui, theme, _kb, done) => {
...
For toggling multiple settings. Use `SettingsList` from `@seaagents/fan-tui` with `getSettingsListTheme()`.

```typescript
import { getSettingsListTheme } from "@seaagents/fan-coding-agent";
import { Container, type SettingItem, SettingsList, Text } from "@seaagents/fan-tui";

fan.registerCommand("settings", {
  handler: async (_args, ctx) => {
...
import { matchesKey, truncateToWidth } from "@seaagents/fan-tui";
