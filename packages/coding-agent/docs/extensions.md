> fan can create extensions. Ask it to build one for your use case.

# Extensions

Extensions are TypeScript modules that extend fan's behavior. They can subscribe to lifecycle events, register custom tools callable by the LLM, add commands, and more.
...
Create `~/.fan/agent/extensions/my-extension.ts`:
...
| Location | Scope |
|----------|-------|
| `~/.fan/agent/extensions/*.ts` | Global (all projects) |
| `~/.fan/agent/extensions/*/index.ts` | Global (subdirectory) |
| `.fan/extensions/*.ts` | Project-local |
| `.fan/extensions/*/index.ts` | Project-local (subdirectory) |
...
To share extensions via npm or git as fan packages, see [packages.md](packages.md).

## Available Imports

| Package | Purpose |
|---------|---------|
| `@itone/fan-coding-agent` | Extension types (`ExtensionAPI`, `ExtensionContext`, events) |
| `@sinclair/typebox` | Schema definitions for tool parameters |
| `@itone/fan-ai` | AI utilities (`StringEnum` for Google-compatible enums) |
| `@itone/fan-tui` | TUI components for custom rendering |
...
After a successful switch or new-session action, fan emits `session_shutdown` for the old extension instance, reloads and rebinds extensions for the new session, then emits `session_start` with `reason: "new" | "resume"` and `previousSessionFile`.
...
After a successful fork, fan emits `session_shutdown` for the old extension instance, reloads and rebinds extensions for the new session, then emits `session_start` with `reason: "fork"` and `previousSessionFile`.
Do cleanup work in `session_shutdown`, then reestablish any in-memory state in `session_start`.
...
Before `tool_call` runs, fan waits for previously emitted Agent events to finish draining through `AgentSession`. This means `ctx.sessionManager` is up to date through the current assistant tool-calling message.
...
```typescript
import { isToolCallEventType } from "@itone/fan-coding-agent";
...
```typescript
import { isBashToolResult } from "@itone/fan-coding-agent";
...
```typescript
import { createLocalBashOperations } from "@itone/fan-coding-agent";
...
pi.on("user_bash", (event, ctx) => {
  ...
  // Option 2: Wrap fan's built-in local bash backend
...
```typescript
import { SessionManager } from "@itone/fan-coding-agent";
...
```typescript
import type { AutocompleteItem } from "@itone/fan-tui";
...
```typescript
import { withFileMutationQueue } from "@itone/fan-coding-agent";
...
**Important:** Use `StringEnum` from `@itone/fan-ai` for string enums. `Type.Union`/`Type.Literal` doesn't work with Google's API.

**Argument preparation:** `prepareArguments(args)` is optional. If defined, it runs before schema validation and before `execute()`. Use it to mimic an older accepted input shape when fan resumes an older session whose stored tool call arguments no longer match the current schema. Return the object you want validated against `parameters`. Keep the public schema strict. Do not add deprecated compatibility fields to `parameters` just to keep old resumed sessions working.
...
- [read.ts](https://github.com/itone/fan-mono/blob/main/packages/coding-agent/src/core/tools/read.ts) - `ReadToolDetails`
- [bash.ts](https://github.com/itone/fan-mono/blob/main/packages/coding-agent/src/core/tools/bash.ts) - `BashToolDetails`
- [edit.ts](https://github.com/itone/fan-mono/blob/main/packages/coding-agent/src/core/tools/edit.ts)
- [write.ts](https://github.com/itone/fan-mono/blob/main/packages/coding-agent/src/core/tools/write.ts)
- [grep.ts](https://github.com/itone/fan-mono/blob/main/packages/coding-agent/src/core/tools/grep.ts) - `GrepToolDetails`
- [find.ts](https://github.com/itone/fan-mono/blob/main/packages/coding-agent/src/core/tools/find.ts) - `FindToolDetails`
- [ls.ts](https://github.com/itone/fan-mono/blob/main/packages/coding-agent/src/core/tools/ls.ts) - `LsToolDetails`
...
```typescript
import { createReadTool, createBashTool, type ReadOperations } from "@itone/fan-coding-agent";
...
For `user_bash`, extensions can reuse fan's local shell backend via `createLocalBashOperations()` instead of reimplementing local process spawning, shell resolution, and process-tree termination.

The bash tool also supports a spawn hook to adjust the command, cwd, or env before execution:

```typescript
import { createBashTool } from "@itone/fan-coding-agent";
...
Tools can provide `renderCall` and `renderResult` for custom TUI display. See [tui.md](tui.md) for the full component API and [tool-execution.ts](https://github.com/itone/fan-mono/blob/main/packages/coding-agent/src/modes/interactive/components/tool-execution.ts) for how tool rows are composed.
...
```typescript
import { keyHint } from "@itone/fan-coding-agent";
...
// Terminal title
ctx.ui.setTitle("fan - my-project");
...
```typescript
import { Text, Component } from "@itone/fan-tui";
...
```typescript
import { CustomEditor, type ExtensionAPI } from "@itone/fan-coding-agent";
import { matchesKey } from "@itone/fan-tui";
...
```typescript
import { highlightCode, getLanguageFromPath } from "@itone/fan-coding-agent";
