...
import { getModel, type OAuthCredentials, type OAuthProvider } from "@seaagents/fan-ai";
...
const AUTH_PATH = join(homedir(), ".fan", "agent", "auth.json");
...
/**
 * Resolve API key for a provider from ~/.fan/agent/auth.json
 *
...
/**
 * Check if a provider has credentials in ~/.fan/agent/auth.json
 */
...
export const FAN_AGENT_DIR = join(homedir(), ".fan", "agent");

/**
 * Get an AuthStorage instance backed by ~/.fan/agent/auth.json
