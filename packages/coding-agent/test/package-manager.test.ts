...
describe("DefaultPackageManager", () => {
	...
	beforeEach(() => {
		previousOfflineEnv = process.env.FAN_OFFLINE;
		delete process.env.FAN_OFFLINE;
	...
	afterEach(() => {
		if (previousOfflineEnv === undefined) {
			...
			process.env.FAN_OFFLINE = previousOfflineEnv;
		}
		vi.restoreAllMocks();
		vi.unstubAllGlobals();
...
Content`,
		...
		it("should auto-discover root markdown skills from .fan skill dirs", async () => {
...
Content`,
		...
		it("should resolve project paths relative to .fan", async () => {
			const extDir = join(tempDir, ".fan", "extensions");
			mkdirSync(extDir, { recursive: true });
		...
		it("should auto-discover project prompts with overrides", async () => {
			const promptsDir = join(tempDir, ".fan", "prompts");
			mkdirSync(promptsDir, { recursive: true });
		...
		it("should resolve directory with package.json fan.extensions in extensions setting", async () => {
			// Create a package with fan.extensions in package.json
			...
			// Should find the extensions declared in package.json fan.extensions
	...
	describe(".agents/skills auto-discovery", () => {
		...
		it("should keep ~/.agents/skills user-scoped when cwd is under home in a non-git directory", async () => {
			...
			try {
				...
				const localAgentDir = join(tempDir, ".fan", "agent");
	...
	describe("ignore files", () => {
		...
		it("should not apply parent .gitignore to .fan auto-discovery", async () => {
			writeFileSync(join(tempDir, ".gitignore"), ".fan\n");

			const skillDir = join(tempDir, ".fan", "skills", "auto-skill");
			mkdirSync(skillDir, { recursive: true });
	...
	describe("resolveExtensionSources", () => {
		...
		it("should handle directories with fan manifest", async () => {
	...
	describe("settings source normalization", () => {
		...
		it("should store project local packages relative to .fan settings base", () => {
			...
			const rel = relative(join(tempDir, ".fan"), projectPkgDir);
			const expected = rel.startsWith(".") ? rel : `./${rel}`;
			expect(settings.packages?.[0]).toBe(expected);
		});
	...
	describe("pattern filtering in fan manifest", () => {
...
export default function(api) { api.registerTool({ name: "test", description: "test", execute: async () => helper() }); }`,
		...
		it("should respect package.json fan.extensions manifest in subdirectories", async () => {
	...
	describe("offline mode and network timeouts", () => {
		it("should update project npm packages using @latest", async () => {
			...
			expect(runCommandSpy).toHaveBeenCalledWith(
				"npm",
				["install", "example@latest", "--prefix", join(tempDir, ".fan", "npm")],
				undefined,
			);
		});
		...
		it("should not fetch npm registry during resolve for installed unpinned packages", async () => {
			const installedPath = join(tempDir, ".fan", "npm", "node_modules", "example");
			mkdirSync(join(installedPath, "extensions"), { recursive: true });
		...
		it("should skip pinned packages when checking for updates", async () => {
			const installedNpmPath = join(tempDir, ".fan", "npm", "node_modules", "example");
