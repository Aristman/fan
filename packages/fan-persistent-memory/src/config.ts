...
export interface RetrievalConfig {
  ...
  incrementalRag: boolean;
  topicShiftThreshold: number;
  maxPromptsWithoutRag: number;
}
...
const CONFIG_DIR = join(homedir(), ".pi", "agent", "memory");
...
export function getProjectDbPath(projectDir: string): string {
  return join(projectDir, ".pi", "memory", "project.db");
}
