...
import type { WsMessage, WsOutgoingMessage } from "@fan/api-gateway/types";
...
export type WsServerMessage = WsConnected | WsPong | WsOutgoingMessage;
...
export class FanWsClient {
	...
	private _scheduleReconnect(): void {
		...
		const base = 1000;
		const max = 30_000;
		const delay = Math.min(base * 2 ** this.retries, max);
		this.retries++;
