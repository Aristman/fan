...
import { Save, X, Cpu, Route, Settings2, RefreshCw, Star, Pencil } from "lucide";
            ...
            ${this.settings.map(
              (s) => html`
                <tr class="border-b border-border/50 hover:bg-foreground/[0.02] transition-colors">
                  ${this.editingId === s.id
                    ? this._renderEditRow(s)
                    : this._renderDisplayRow(s)}
                </tr>
              `,
            )}
          </tbody>
    ...
    return html`
      <div class="rounded-lg border border-border p-3 space-y-1.5 transition-colors">
        ...
        <div class="flex items-center justify-between">
          ...
          <span
            class="text-[10px] font-medium px-1.5 py-0.5 rounded ${rule.enabled
  ...
  private _renderModelsTab() {
    ...
    return html`
      <div class="grid gap-2">
        ${Array.from(this.modelsByProvider.entries()).map(
          ([provider, models]) => this._renderProviderCard(provider, models),
        )}
      </div>
    `;
  }
