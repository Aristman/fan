...
### Таймаут runAgent (per-mission)

Поле `runagent_timeout_min` в frontmatter `MISSION.md` задаёт таймаут одного запуска `runAgent` (минуты). Диапазон: 1–480, дефолт: 30 (если поле отсутствует или невалидно). Поле прописывается при `fan mission init` (включено в шаблоны `default` и `refactor`).

| Значение | Рекомендация |
|----------|--------------|
| 30 | Дефолт; подходит для большинства миссий |
| 90–120 | Жирные пункты ROADMAP (30+ мин verify, тяжёлые compute-задачи) |

Поле читается regex'ом из сырого `MISSION.md` при `attachMission` (не через YAML-парсер), поэтому может находиться в любом месте frontmatter.
