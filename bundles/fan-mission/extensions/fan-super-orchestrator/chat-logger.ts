...
export function sendSessionStartMessage(opts: SessionStartOpts): string {
  ...
  console.log(`[${opts.correlationId}] ${opts.role}:${opts.roleProfile} initialized, depth=${opts.depth}, lineage_len=${opts.lineageLen}`);
  if (opts.chatEmitter) {
...
export function logHandlerEntry(opts: HandlerEntryOpts): string {
  ...
  console.error(`[fan-super-orchestrator] delegate handler entered: corr=${opts.correlationId}, packages=${opts.packages}`);
  if (opts.chatEmitter) {
    opts.chatEmitter(msg);
  }
  return msg;
