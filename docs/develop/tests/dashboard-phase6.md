...
- [ ] Удаление сессии (красная корзина на hover) → confirm → удаление
- [ ] Клик по сессии → переход в чат
...
**Проверить:**
- [ ] Сообщение пользователя появляется справа (бабл)
- [ ] Индикатор "Thinking..." пока агент обрабатывает
- [ ] Ответ ассистента стримится посимвольно слева
- [ ] Markdown рендерится (bold, inline code, code blocks)
- [ ] Tool calls отображаются как сворачиваемые блоки (bash, read и т.д.)
...
**Проверить:**
- [ ] События через WS приходят в dashboard (агент работает на сервере)
- [ ] Перезапустить API Gateway → dashboard пытается реконнектиться
- [ ] После перезапуска: статус-индикатор в header переключается обратно на зелёный
...
**Проверить:**
- [ ] Отображаются карточки бюджетов по провайдерам/периодам
...
**Проверить:**
- [ ] Три таба: Model Settings, Routing Rules, Available Models
...
**Проверить:**
- [ ] Открывается диалог с бэкдропом
- [ ] Tab "Connection": текущий URL и token (замаскированный), кнопка Test Connection
- [ ] Tab "API Tokens": список токенов с name, created, last used
- [ ] Generate New Token: ввести имя → сгенерировать → токен показан ОДИН раз
- [ ] Copy token → буфер обмена
- [ ] Revoke token → confirm → удалён из списка
- [ ] Закрытие диалога (крестик или бэкдроп)
