...
- **Rendering:** JCEF (Chromium) + JEditorPane fallback
...
## Phase 1: JCEF Rendering Engine (Visual Core)
...
- Auto-fallback на JEditorPane если JCEF недоступен
...
- `FanHtmlDocumentManager` — единый API поверх JCEF (fallback JEditorPane)
...
- Auto-fallback: JCEF error → `fallbackToEditorPane()`
...
|---|----------------------------------------------------------------|--------------------|--------------------------------------------------------------|
| 1 | **JCEF availability** — не все JetBrains runtime включают JCEF | Rendering fallback | JEditorPane fallback (ORF pattern)                           |
| 2 | **Kotlin upgrade** 1.9.24 → 2.1.0 — compilation issues         | Блокировка         | Инкрементальный тест                                         |
| 3 | **Bundle size** — JSoup + JCEF resources                       | Больше плагин      | JSoup ~1MB, приемлемо                                        |
| 4 | **Performance** — JCEF startup vs Swing                        | Медленный старт    | Lazy init, script queuing                                    |
| 5 | **Coroutines** — зависимость от IntelliJ bundled version       | Ограничения        | `executeOnPooledThread` для Swing UI, no explicit dependency |
