...
12. **Production runAgent захват ответа:** дефолтный runAgent в fan-mission/index.ts — fire-and-forget sendUserMessage (возвращает пустой response, cost 0). Для полноценной итерации нужен захват raw-ответа LLM + usage через хук agent_end (one-shot listener + timeout + защита от чужого хода через MissionLock/isIdle). Сейчас promise-теги из ответа агента не попадают в контур в production.
13. **Виджет ui-заглушка:** mission-widget в index.ts использует no-op ui (render/toggle), т.к. при factory нет ctx.ui. Для реальной отрисовки виджета нужен доступ к ctx.ui (например через сохранение последнего ctx из session_start).
14. **Auto-tick loop:** fan-scheduler доставляет tick-промпт LLM-у, но не вызывает сам missionLoop.tick(). Нужен модуль авто-тика который гоняет 7-шаговый цикл (с обработкой awaiting_decision).
