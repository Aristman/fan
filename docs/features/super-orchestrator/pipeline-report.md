...
32. **#32 Planning-итерация без результата крутится до budget_exhausted (0.7.0):** если Goal заполнен, но агент не добавляет пункты в ROADMAP, planning-тики повторяются без коммитов вплоть до исчерпания бюджета. Сделать: лимит подряд идущих пустых planning-итераций (2–3) → DECIDE оператору.
