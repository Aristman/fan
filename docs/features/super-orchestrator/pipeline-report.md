...
**Следующие шаги:** ручное тестирование (подробная инструкция: `manual-testing.md` в этом каталоге) → бэклог (см. ниже) → мерж → релиз.
...
31. **#31 Структурированный context для write-воркеров (fan-orchestrator):** механизм инжекции тип-агностичен (subagent-runner.js:660, mergeContext во всех режимах), но заполнение context — решение LLM-координатора; без него воркер переисследует известное координатору (расход токенов). Сделать: warning в результате delegate_task при отсутствии context у implement/bug-fix/tests-impl/docs-impl + config-флаг requireContextForWriteWorkers.
32. **#32 ЗАКРЫТО (07cb76c, fan-mission 0.7.2):** Planning-итерация без результата крутится до budget_exhausted (0.7.0): если Goal заполнен, но агент не добавляет пункты в ROADMAP, planning-тики повторяются без коммитов вплоть до исчерпания бюджета. Сделано: emptyPlanningStreak ≥2 → awaiting_decision.

### #33 — git commit failed с пустым stderr, когда агент коммитит сам
- **Найдено:** 2026-08-18, gmail-watch итерация 5 (ручное тестирование)
