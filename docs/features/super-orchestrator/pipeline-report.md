...
14. **Auto-tick loop** — ⚠️ ЧАСТИЧНО: холостые тики без миссии устранены (`cf45deb`, fan-scheduler 0.1.1 — детектор активной миссии, без неё не тикает + реальный missionDir). Осталось: мост который вызывает сам missionLoop.tick() (сейчас тик шлётся промптом LLM-у).
