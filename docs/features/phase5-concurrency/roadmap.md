...
- **Зависимости:** (none)
- **TDD-тесты:**
  - [ ] **TC-F-5.1-1:** Known consumer found and categorized
  ...
  - [ ] **TC-F-5.1-2:** No false negatives for critical paths
...
- **TDD-тесты:**
  - [ ] **TC-F-5.2-1:** agent-session-runtime.ts found
  ...
  - [ ] **TC-F-5.2-2:** Extensions checked for chdir usage
...
- **TDD-тесты:**
  - [ ] **TC-F-5.3-1:** Tool resolves path relative to session cwd
  ...
  - [ ] **TC-F-5.3-2:** process.cwd() not called inside tools
  ...
  - [ ] **TC-F-5.3-3:** Parallel sessions have independent tool contexts
    - *Условие:* Two sessions with different cws started; both execute tools reading/writing files
...
- **TDD-тесты:**
  - [ ] **TC-F-5.4-1:** switchSession without process.chdir call
  ...
  - [ ] **TC-F-5.4-2:** Multiple sequential switchSession preserve isolation
  ...
  - [ ] **TC-F-5.4-3:** Teardown of old context releases resources
...
- **Критерии приёмки:**
  ...
  3. Backward compatibility: caller API unchanged (`switchSession(path)` still works but now needs sessionId)
...
- **TDD-тесты:**
  - [ ] **TC-F-5.5-1:** Enqueue creates/extends JSONL file
  ...
  - [ ] **TC-F-5.5-2:** Dequeue returns first-in-first-out
  ...
  - [ ] **TC-F-5.5-3:** File survives server restart
...
- **TDD-тесты:**
  - [ ] **TC-F-5.6-1:** Queues restored on server start
  ...
  - [ ] **TC-F-5.6-2:** Empty server starts clean
...
- **TDD-тесты:**
  - [ ] **TC-F-5.7-1:** Null scope = full access (backward compat)
  ...
  - [ ] **TC-F-5.7-2:** Scoped token limited to project
  ...
  - [ ] **TC-F-5.7-3:** Exact project path match granted
...
#### ☐ F-5.8: E2E — Два клиента, параллельные сессии разных проектов, целостность JSONL
...
- **TDD-тесты:**
  - [ ] **TC-F-5.8-1:** Parallel sessions isolated — no race conditions
  ...
  - [ ] **TC-F-5.8-2:** Process cwd invariant maintained
  ...
  - [ ] **TC-F-5.8-3:** Message queue survives restart mid-execution
...
└── Этап 5.3: Перспективное                                    │
    └── F-5.7: Per-project tokens (API)                        │
                                                              │
└── E2E                                                       │
    └── F-5.8: Параллельные сессии ← ALL above                │
...
- [ ] ☐ F-5.4 Remove process.chdir() from runtime.switchSession()
- [ ] ☐ F-5.5 Persistent message queue — core operations
- [ ] ☐ F-5.8 E2E: параллельные сессии + integrity
