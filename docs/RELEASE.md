...
патче после релиза 1.0.0. `npm run build` больше не меняет поля `version` в
`package.json` пакетов. Каждый пакет обновляется вручную.
...
только компилирует пакеты и не изменяет `package.json`.
