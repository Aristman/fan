...
Версионирование FAN-бинарника использует **два независимых семвера**:

| Понятие | Источник | Пример | Назначение |
|---------|---------|--------|-----------|
| **Версия FAN-бинарника** (`FAN_VERSION`) | Корневой `package.json` | `2.0.0` | Имя архива (`fan-2.0.0-linux-x64.tar.gz`), поле `latest` в `manifest.json`, версия, показываемая в `fan --version` |
| **Версия npm-пакета** (`CODING_AGENT_VERSION`) | `packages/coding-agent/package.json` | `1.0.3` | Метаданные `@seaagents/fan-coding-agent` в npm-публикации, `package.json` внутри бинарника |

Это два независимых семантических версионирования. Корневой `package.json`
отвечает за релизные маркеры всего проекта (FAN). `packages/coding-agent/package.json`
отвечает за версию npm-пакета `@seaagents/fan-coding-agent`.
...
### Версия FAN-бинарника и версия npm-пакета

В FAN существуют **два независимых семантических версионирования**:

1. **Версия FAN-бинарника** (`FAN_VERSION`) — читается из корневого `package.json`
   (workspace root). Используется для имени архива (`fan-2.0.0-linux-x64.tar.gz`),
   поля `latest` в `manifest.json` и версии, показываемой в `fan --version`.
   Это релизный маркер всего проекта FAN.

2. **Версия npm-пакета** (`CODING_AGENT_VERSION`) — читается из
   `packages/coding-agent/package.json`. Используется в метаданных
   `@seaagents/fan-coding-agent` при публикации в npm. Внутри бинарника
   копируется `package.json` именно пакета `@seaagents/fan-coding-agent`
   (с его собственной версией, независимой от корневой).

Эти две версии не обязаны совпадать. Изменения в других пакетах не обязаны
вести к бампу `@seaagents/fan-coding-agent`. Бинарник (сборка) пересобирается
с корневой версией, а npm-пакет публикуется со своей.
...
- Версия корневого `package.json` (workspace root) — **основная версия FAN-бинарника**.
  Используется в имени архива, `manifest.json` и `fan --version`.
- Версия `@seaagents/fan-coding-agent` — это версия npm-пакета, независимая от корневой.
  Бампится когда меняется сам пакет или нужна новая npm-публикация.
- Версия FAN-бинарника бампится в корневом `package.json`.
  Бинарник пересобирается под этой версией.
...
| `scripts/release-binaries.sh` | Релизы 1.0.0+ | Независимое: `FAN_VERSION` из корневого `package.json`, `CODING_AGENT_VERSION` из `packages/coding-agent/package.json` | **Только для релизов** |
...
- Версия FAN-бинарника (**FAN_VERSION**) и версия npm-пакета `@seaagents/fan-coding-agent`
  (**CODING_AGENT_VERSION**) — это два независимых семвера. Корневая версия отвечает
  за релизный маркер проекта, coding-agent версия — за npm-публикацию.
- В `manifest.json` указывается только версия бинарника (одно поле `latest`).
  Версия npm-пакета не попадает в манифест.
