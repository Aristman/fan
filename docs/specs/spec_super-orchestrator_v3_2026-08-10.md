...
## Метаданные
- **Дата:** 2026-08-10
- **Статус:** Спецификация v3
- **Версия:** 3.0
- **Заменяет:** `spec_super-orchestrator_2026-08-10.md` (v2)
...
- **Источники:**
  ...
  - `.fan/reports/spec-comparison-v1-v2.md` — отчёт сравнения v1/v2: 18 пробелов, 3 блока для переноса
...
- REST/WS drain API — нет эндпоинта для graceful-паузы извне (~20 LOC, `packages/api-gateway/src/http-server.ts`)
...
- Аутентификация между узлами — нет протокола обмена токенами L0↔L1 (~25 LOC ядро, сидинг FAN_NODE_TOKEN)
...
**FSM статусов миссии — таблица переходов:**

| Текущий статус → | active | paused | completed | aborted | failed | budget_exhausted |
|---|:---:|:---:|:---:|:---:|:---:|:---:|
| **active** | — | I1 (оператор/watchdog) | контур (все этапы ✓) | I0 (оператор) | контур (circuit breaker) | бюджет (BudgetTracker) |
| **paused** | `/mission:resume` (оператор) | — | ✗ | I0 (оператор) | ✗ | ✗ |
| **completed** | ✗ | ✗ | — (терминальный) | ✗ | ✗ | ✗ |
| **aborted** | оператор (явный перезапуск) | ✗ | ✗ | — | ✗ | ✗ |
| **failed** | оператор (явный перезапуск) | ✗ | ✗ | ✗ | — | ✗ |
| **budget_exhausted** | пополнение бюджета (оператор) | ✗ | ✗ | ✗ | ✗ | — |

**Правила:**
- `completed` — терминальный статус, переходов нет
- `failed`/`aborted` → `active` только по явному действию оператора
- `budget_exhausted` → `active` после пополнения бюджета в `MISSION.md`
- Любой не-терминальный статус → `aborted` по I0 (оператор, немаскируемый)
- Инициаторы: оператор (TUI/REST), контур (автоматически), watchdog, BudgetTracker
...
2. **Оценщик** — гибрид LLM + детерминированная арифметика. LLM оценивает каждый критерий по фиксированной рубрике (промпт-рубрика в конфиге fan-mission) шкалой 0.0–1.0. Взвешенная сумма вычисляется кодом:
   ```
   score = 0.3 × соответствие + 0.2 × ценность + 0.2 × (1 − риск) + 0.3 × (1 − стоимость)
   ```
   Сырые оценки и итоговый score логируются в `BACKLOG.md` (воспроизводимость = детерминированная арифметика + зафиксированные входные оценки). Пороги: ≥0.7 — в ROADMAP; 0.5–0.7 — DECIDE оператору; <0.5 — отклонить с записью причины в `DECISIONS.md`.
...
**Формат записи кандидата в `BACKLOG.md`:**

| id | дата | идея | источник | соответствие | ценность | риск | стоимость | score | статус |
|----|------|------|----------|:---:|:---:|:---:|:---:|:---:|--------|
| idea-007 | 2026-08-10 | Рефакторинг auth middleware на JWT | итерация #5 | 0.9 | 0.8 | 0.3 | 0.4 | 0.75 | ROADMAP |
...
| I1 | **Осушение** (drain) | «Закончи текущий ход и встань»: graceful-пауза на границе итерации | SIGTERM + drain | Флаг `drainAfterCurrentTurn` в `agent-session.ts` 🆕 ~30 LOC + REST `POST /api/sessions/:id/drain` 🆕 ~20 LOC |
...
|----------|---------|----------|
| Watchdog (зависший tool call) | I0–I1 | Таймер `watchdogTimeoutMs` (дефолт 720000 = 12 мин) на `tool_execution_start` без `tool_execution_end`; сбрасывается любым `tool_execution_update` 🆕 |
...
3. Маршрутизация:
   ...
   - I1 → `POST /api/sessions/:id/drain` (установка флага drain) + запись `STATE.md` (status: draining); *`draining` — переходное состояние (не основной статус FSM), означает «завершить текущий tool call, затем → `aborted`»*
   ...
   - I4 → проверка условий продолжения + `POST /api/sessions/:id/messages` с тик-промптом (см. формат тика ниже)
...
**Правила коллизий прерываний:**
- **(a)** I0 во время I1-drain — немедленный abort, drain отменяется
- **(b)** Несколько I2 — FIFO с дедупликацией по хешу (источник + содержимое) за окно 5 минут
- **(c)** I2 и I3 одновременно — I2 доставляется в текущий ход, I3 остаётся в очереди
- **(d)** I4-тик во время активной итерации — откладывается до `agent_end` (очередь followUp)

**Формат тика (fan-scheduler):** тик — полноценный промпт, шаблон хранится в конфиге fan-scheduler (`tickPrompt`, строка с плейсхолдерами `{missionDir}`, `{date}`). Дефолтный текст:

> Тик контура миссии. Прочитай STATE.md, ROADMAP.md и BACKLOG.md в {missionDir}, проверь git log. Реши по алгоритму контура: запустить следующую итерацию, сгенерировать идеи или остаться в ожидании. Ответь одним из: ITERATE:<id пункта>, GENERATE, IDLE:<причина>.
...
**Парсер тегов обещаний:** регулярное выражение `/<promise>(COMPLETE|BLOCKED(?::[^\n<]+)?|DECIDE(?::[^\n<]+)?|FAILED(?::[^\n<]+)?)<\/promise>/`. Правила:
- Теги внутри fenced code block (```` ``` ````) игнорируются
- При нескольких тегах — последний валидный
- `BLOCKED` без причины → причина «не указана»
- Отсутствие тега по завершении итерации → статус `unknown` + эскалация оператору (I3)
...
- Лимит очереди: 50 сообщений (конфигурируемый, переполнение → явная ошибка отправителю, не молчаливая потеря) 🆕 (~20 LOC в `agent.ts`)
- Dead-letter: сообщения старше 1 часа без доставки → `DECISIONS.md` как «пропущенное прерывание»
- При drain (I1): накопленные I3 (followUp) сохраняются и обрабатываются при resume
- При переполнении очереди (50) во время паузы — новые сообщения отклоняются с ошибкой отправителю
- При abort (I0): очереди очищаются с записью в журнал миссии
...
**Исчерпание пула портов:** при исчерпании пула 7001–7099 — ошибка порождения узла + эскалация I3 оператору. Диапазон конфигурируется (`portRangeStart`/`portRangeEnd` в конфиге оркестратора).
...
- При исчерпании пула портов — явная ошибка + эскалация

#### 3.3.1a Аутентификация между узлами

**Проблема:** L0 должен вызывать REST/WS дочернего `fan server`, который требует ClientToken-аутентификацию (`packages/api-gateway/src/auth.ts`). Без протокола обмена токенами этап 2 нереализуем.

**Решение:**
- При порождении L0 генерирует токен узла: `randomBytes(32).toString("hex")` (тот же формат, что ClientToken, ссылка `packages/api-gateway/src/auth.ts`)
- Токен передаётся дочернему процессу через переменную окружения `FAN_NODE_TOKEN`
- Дочерний `fan server` при старте: если `FAN_NODE_TOKEN` задан — регистрирует его в своём ClientToken-хранилище (Prisma) до поднятия HTTP-сервера. Изменение ядра: сидинг токена из env в server bootstrap, ~25 LOC
- L0 использует тот же токен для всех вызовов REST/WS дочернего узла (Bearer / `?token=`)
- Токен живёт только в памяти процесса-родителя и БД ребёнка; при остановке узла L0 отзывает токен (`DELETE /api/tokens/:id`) перед SIGTERM
- `FAN_NO_AUTH` остаётся выключенным для дочерних узлов даже на localhost (узлы — не интерактивные клиенты). L0 обязан передать дочернему процессу `FAN_NO_AUTH=0` в env спавна явно, так как `packages/coding-agent/src/main.ts:1255-1256` по умолчанию включает `FAN_NO_AUTH=1` в server mode и дочерний процесс наследует env; сидинг `FAN_NODE_TOKEN` выполняется до поднятия HTTP-сервера и проверки auth.

**Критерии приёмки:**
- Дочерний узел принимает запросы только от родителя с валидным токеном
- При остановке узла токен отзывается
- `FAN_NO_AUTH` не влияет на дочерние узлы
...
```json
{
  "message": "{\"work_package\":{\"task\":\"Реализовать middleware авторизации\",\"correlationId\":\"mission-uuid/L1/node-3\",\"depth\":1,\"spawnBudget\":4,\"tokenBudget\":50000,\"costBudgetUsd\":2.00,\"maxRetries\":2,\"toolManifest\":[\"read\",\"write\",\"edit\",\"bash\",\"grep\",\"find\",\"ls\"],\"deadline\":\"2026-08-10T14:00:00Z\",\"verificationCommand\":\"npm run test:unit\",\"context\":{\"parentSummary\":\"Миссия рефакторинга auth, этап 2 из 4\",\"relevantFiles\":[\"src/middleware/auth.ts\",\"tests/auth.test.ts\"],\"constraints\":[\"не менять public API\"]}}}",
...
- `maxRetries` — максимум повторных попыток при провале (дефолт 2); исчерпание → пакет помечается `failed`, эскалация оператору (I3), контур не перезапускает сам
...
**Валидация отчёта:**
- Нет `VERDICT` в ответе → `status:"unknown"`, одна повторная попытка (считается в `maxRetries`), затем эскалация оператору
- `PARTIAL` без описания выполненных частей → запрос уточнения узлу (steer) или трактовка как FAIL при повторе
- Отчёт без `usage` → запрос повторной отправки; при повторном провале — `status:"unknown"`
...
- Отчёт без VERDICT → `status:"unknown"` + эскалация после исчерпания ретраев
...
2. При порождении L1: выделение по формуле `min(0.8 × budget_remaining / planned_children, per_hop_ceiling)`, где `per_hop_ceiling` — конфиг (дефолт 30000 токенов). Резерв 20% остаётся родителю на синтез и ретраи. Если `budget_allocated > budget_total` → отказ в порождении
3. L1 отчитывается: при `agent_end` → `usage` добавляется в `budget_consumed`. Неизрасходованный остаток возвращается в пул родителя при отчёте узла
...
**Двойной бюджет (токены + USD):** оба лимита жёсткие и независимые; срабатывает первый достигнутый. Алерты на 80% по каждому измерению отдельно.

**Исчерпание бюджета узла:** дочерний узел отслеживает свой выделенный бюджет через BudgetTracker; при исчерпании — завершает текущий ход (drain), фиксирует состояние, возвращает отчёт `status:"budget_exhausted"` с частичными результатами. Родитель решает: пополнить (повторный пакет с новым бюджетом) или закрыть ветку.
...
**Crash vs штатное завершение:** при штатном прерывании/завершении итерации контур пишет в STATE.md маркер `interrupted: true` (или статус итерации `interrupted`). Отсутствие маркера при `status: active` на тике = crash recovery → итерация перезапускается с последнего чекпоинта (git).

**Максимальная потеря при любом сценарии** — одна незавершённая итерация. Всё зафиксированное (коммиты, STATE.md, журнал дерева) переживает любой сбой. Доска задач узла также персистится снапшотом `TaskManager` в сессионный JSONL (custom message) и восстанавливается при `session_start` / `switchSession` (этап 1, см. фичу F-48 roadmap `mission-validation-1` и backlog `docs/backlogs/tasklist-persistence-backlog.md`); до реализации F-48 доска задач эфемерна и теряется при рестарте FAN.
...
3. **Сторожевой таймер (watchdog)** — на каждый `tool_execution_start`; нет `tool_execution_end` за `watchdogTimeoutMs` (дефолт 720000 мс = 12 минут, конфигурируется в config.json оркестратора/fan-mission) → сигнал I1 + диагностика. Таймер сбрасывается любым событием `tool_execution_update` — длинные, но живые tool call'ы не убиваются (паттерн stall timer из `extensions/fan-orchestrator/subagent-runner.js`). В порождённых воркерах stall timer уже работает (`subagent-runner.js`); нужен аналог на уровне runtime. Реализуется в `packages/coding-agent/src/core/agent-session.ts` (~80 LOC).
...
**Поведение:**
- Коллапсируемый блок над редактором (по образцу task widget оркестратора)
- Авто-скрытие при отсутствии активной миссии
- При наличии активной миссии — всегда виден, можно свернуть вручную
...
| 5 | Режим осушения (drain-флаг) | `packages/coding-agent/src/core/agent-session.ts` | ~30 | P1 |
| 6 | Drain API эндпоинт | `packages/api-gateway/src/http-server.ts`, `packages/api-gateway/src/types.ts` (WsIncomingMessage) | ~20 | P0 |
| 7 | Сидинг FAN_NODE_TOKEN из env | `packages/coding-agent/src/cli/server-command.ts` (bootstrap) | ~25 | P2 |
| 8 | Checkpoint API (явный) | `packages/coding-agent/src/core/agent-session.ts` | ~100 | P2 |
| 9 | Бюджет на итерацию | `packages/model-manager/src/budget.ts` | ~60 | P2 |
| 10 | Продюсер `budget_alert` | `packages/api-gateway/src/ws-handler.ts` | ~40 | P0 |
| 11 | CLI mission-command (`fan mission init/tree`) | `packages/coding-agent/src/cli/mission-command.ts` | ~300 | P1 |
| 12 | Персистентность таск-листа (orchestrator extension + session JSONL) | `extensions/fan-orchestrator/task-manager.js`, `packages/coding-agent/src/core/agent-session-runtime.ts` (custom messages) | ~100 | P1 |

**Итого изменений ядра:** ~905 LOC

> *Сноска: P0/P1/P2 = порядок внедрения по этапам, не важность; важность отражена в MoSCoW (раздел 11).*

**Совместимость с существующими клиентами:** лимит очередей (50) конфигурируемый, при переполнении — явная ошибка отправителю (не молчаливая потеря). Watchdog не убивает живые вызовы (сброс на `tool_execution_update`, см. §3.5). Новые эндпоинты аддитивны; существующие клиенты (TUI, Dashboard, IDEA plugin) не ломаются. Feature flag `FAN_MISSION_ENABLED` покрывает только миссионную функциональность.
...
| POST | `/api/sessions/:id/drain` | Drain сессии (I1): устанавливает флаг `drainAfterCurrentTurn`; ответ 202 `{status:"draining"}`, повторный вызов идемпотентен | api-gateway | P0 |
...
| `WS /api/ws/:sessionId` | Поддержка команд `abort` и `drain` (сейчас только ping/subscribe, расширение `WsIncomingMessage` в `packages/api-gateway/src/types.ts`) |
...
### 6.6 Стратегия тестирования

**Общий объём:** ~950 LOC тестов (этап 0: ~500 LOC, этапы 1–3: ~450 LOC)

**Unit-тесты ядра (P0):**
- **Watchdog:** таймер запуск/сброс на `tool_execution_update`, срабатывание при истечении `watchdogTimeoutMs` (720000 мс), отмена при `tool_execution_end`
- **Детектор циклов:** семантическое сходство (тот же инструмент + параметры + ошибка), порог 2 повтора, сброс после успешного вызова
- **Drain-флаг:** завершение текущего хода, запрет новых ходов после установки, идемпотентность повторной установки
- **Лимиты очередей:** переполнение 50 → ошибка отправителю, конфигурируемость лимита
- **REST abort/drain эндпоинты:** аутентификация (ClientToken), идемпотентность drain, статус-коды (202)

**Unit-тесты fan-mission:**
- **Парсер STATE.md:** 3 секции («Сделано», «Блокеры», «Следующие шаги»), лимит 5 KB, обработка малформированных файлов
- **FSM статусов:** таблица переходов из §3.1.2 (валидные/невалидные переходы, инициаторы)
- **Парсер тегов обещаний:** regex из §3.2.4, edge cases: тег без причины, тег в fenced code block, несколько тегов, отсутствие тега
- **Скоринг идей:** детерминированная арифметика поверх фиксированных LLM-оценок, воспроизводимость, пороги (0.7 / 0.5)

**Интеграционные тесты:**
- Синтетическая миссия end-to-end: init → тик → итерация → commit → STATE.md
- Mock LLM-провайдер с фикстурными ответами (детерминированность)
- Тест прерываний: steer → агент учитывает, drain → завершает ход

**e2e-тесты:**
- pause → steer → resume через API
- Дерево L0→L1: порождение, отчёт, бюджет

**Фикстуры:** каталог `tests/fixtures/mission/` с образцами файлов миссии (MISSION.md, STATE.md, ROADMAP.md, BACKLOG.md)
...
| 0.7 | Drain mode (ядро, `agent-session.ts`) + Drain API эндпоинт (api-gateway, `http-server.ts`) | ~50 | Низкая |
| 0.8 | Расширение `fan-scheduler` (тики I4) | ~175 | Низкая |
| 0.9 | Расширение `fan-webhook` (слушатель вебхуков) | ~250 | Низкая |
| 0.10 | Стратегия тестирования: unit-тесты ядра P0 + fan-mission + интеграционные + фикстуры | ~500 | Средняя |
...
- Изменения ядра P0: REST abort (~50 LOC), лимиты очередей (~20 LOC), watchdog (~80 LOC), детектор циклов (~80 LOC), продюсер `budget_alert` (~40 LOC), drain API (~20 LOC)
- Изменения ядра P1: drain-флаг (~30 LOC)
- Расширения: `fan-scheduler`, `fan-webhook`
- Стратегия тестирования: unit + интеграционные + фикстуры (~500 LOC)
- Суммарно: ~2145 LOC
...
| 1.8 | Персистентность таск-листа: JSONL-snapshot TaskManager, deserialize, restore-семантика (in_progress→pending+recovered), тесты | ~150 | Низкая |
| **Субитог этапа 1** | | **~1100** | |
...
- Персистентность таск-листа оркестратора: снапшоты `TaskManager` в сессионный JSONL, десериализация с восстановлением blocks/blockedBy, restore-семантика `in_progress`→`pending`+`recovered` (~150 LOC, ~100 ядро + ~50 расширение)
...
- Доска задач восстанавливается после перезапуска процесса (3 задачи в разных статусах → `list_tasks` корректен, `blocks`/`blockedBy` целы); `in_progress` после restore помечен `recovered: true` и переведён в `pending`; backward compat — сессии без snapshot дают пустую доску без ошибок
...
| Шаг | Задача | LOC | Сложность |
|-----|--------|-----|-----------|
| 2.1 | Process manager (порты, запуск, health-check, kill-switch) + аутентификация между узлами (FAN_NODE_TOKEN, сидинг из env, ~25 LOC ядра) | ~425 | Средняя |
...
|------|------|-------------|---------------------|
| 0 | 2–3 нед | Миссионный контур (один узел) + P0-ядро + scheduler + webhook + тесты | ≥10 итераций, состояние в файлах, REST abort <1 сек |
| 1 | 1.5–2 нед | Валидация: прерывания + генерация идей + метрики | I0 <1 сек, DECIDE работает, метрики в пределах |
| 2 | 3–4 нед | HTTP-иерархия (глубина 2, MVP ~3 нед) + auth узлов | Дерево L0→L1, бюджет, журнал, kill-switch, auth |
| 3 | 2–3 нед | Глубина 3–4, манифесты, визуализация | Dashboard, глубина 3–4, валидация |
| **Итого** | **9–12 нед** | **Полная система (~7180 LOC, Приложение B)** | **Миссия глубины 3–4 на реальном эпике** |
...
- [ ] Персистентность таск-листа оркестратора (JSONL-snapshot `TaskManager`, восстановление после перезапуска процесса; подробности — backlog `docs/backlogs/tasklist-persistence-backlog.md`)
...
2. **Этап 0, шаг 0.4:** Добавить REST abort + drain API эндпоинты в API Gateway (~70 LOC) — закрытие красного пробела К1
3. **Этап 0, шаг 0.5:** Лимиты очередей + watchdog в ядре (~100 LOC)
4. **Этап 0, шаг 0.6:** Детектор циклов в ядре (~80 LOC)
5. **Этап 0, шаги 0.1–0.3:** Mission loop + CLI + slash-команды + TUI-виджет (~900 LOC)
6. **Этап 0, шаги 0.8–0.9:** fan-scheduler + fan-webhook (~425 LOC)
7. **Этап 0, шаг 0.10:** Стратегия тестирования: unit + интеграционные + фикстуры (~500 LOC)
8. **Валидация:** Прогон на синтетической миссии (генерация документации по монорепо)
9. **Наблюдение:** 2 недели после этапа 0 — сбор метрик, корректировка перед этапом 1
10. **Этап 2, шаг 2.1:** Аутентификация между узлами (FAN_NODE_TOKEN) — закрытие красного пробела К2
...
## Приложение B: Сводная карта модулей и LOC

| Модуль | Статус | Тип работ | Новый код | Изменения ядра | Итого | Приоритет |
|--------|--------|-----------|-----------|----------------|-------|-----------|
| Очереди steer/followUp | ✅ есть | — | — | — | — | — |
| Восстановление + компактификация | ✅ есть | — | — | — | — | — |
| `fan server` как узел | ✅ есть | — | — | — | — | — |
| 27 событий расширений | ✅ есть | — | — | — | — | — |
| Stall timer (прообраз watchdog) | ✅ есть | — | — | — | — | — |
| Маркер `VERDICT` | ✅ есть | — | — | — | — | — |
| REST/WS abort | 🆕 | Ядро | — | ~50 | ~50 | P0 |
| Лимиты очередей | 🆕 | Ядро | — | ~20 | ~20 | P0 |
| Watchdog | 🆕 | Ядро | — | ~80 | ~80 | P0 |
| Детектор циклов | 🆕 | Ядро | — | ~80 | ~80 | P0 |
| Продюсер `budget_alert` | 🆕 | Ядро | — | ~40 | ~40 | P0 |
| Drain API эндпоинт | 🆕 | Ядро | — | ~20 | ~20 | P0 |
| Режим осушения (drain-флаг) | 🆕 | Ядро | — | ~30 | ~30 | P1 |
| Сидинг FAN_NODE_TOKEN | 🆕 | Ядро | — | ~25 | ~25 | P2 |
| Чекпоинты поверх `fork()` | 🆕 | Ядро | — | ~100 | ~100 | P2 |
| Бюджет на итерацию | 🆕 | Ядро | — | ~60 | ~60 | P2 |
| fan-mission | 🆕 | Расширение | ~1400 | — | ~1400 | P1 |
| fan-scheduler | 🆕 | Расширение | ~175 | — | ~175 | P1 |
| fan-webhook | 🆕 | Расширение | ~250 | — | ~250 | P1 |
| HTTP-иерархия узлов | 🆕 | Расширение | ~2350 | — | ~2350 | P2 |
| Dashboard-компоненты | 🆕 | Dashboard | ~700 | — | ~700 | P2 |
| Сбор метрик (fan-mission) | 🆕 | Расширение | ~100 | — | ~100 | P1 |
| CLI mission-command (ядро) | 🆕 | Ядро | — | ~300 | ~300 | P1 |
| Тесты | 🆕 | — | ~950 | — | ~950 | — |
| Документация | 🆕 | — | ~300 | — | ~300 | — |
| Персистентность таск-листа оркестратора (этап 1) | 🆕 | Расширение + ядро | ~50 | ~100 | ~150 | P1 |
| **Итого** | | | **~6275** | **~905** | **~7180** | |
