...
(function () {
    ...
    const storeData = [
        // ====== EXTENSIONS ======
        {
            ...
            type: 'extension',
            icon: '[ORC]',
            description: 'Мульти-агентная система декомпозиции и координации задач. Запускает 8 типов воркеров: explore, plan, implement, verify, bug-fix, code-research, tests-impl, docs-impl.',
            details: 'Управляет пулом из 8 типов специализированных воркеров. Поддерживает parallel, chain и single режимы запуска. Встроенная система задач с блокировками зависимостей и автоматическим отслеживанием прогресса. Каждый воркер запускается в изолированном процессе с RPC-коммуникацией через JSONL по stdin/stdout. Координатор автоматически декомпозирует сложные задачи на подзадачи.',
            tools: [
            ...
            commands: [
                ...
                '/orchestrator status — статус системы',
                ...
                '/plan <задача> — исследовать и спланировать',
            ...
            usage: 'Установи через fan store install fan-orchestrator. Затем запусти /orchestrator init для настройки моделей и воркеров. После этого координатор будет автоматически декомпозировать сложные задачи.'
        },
        {
        ...
        },
        {
            ...
            type: 'extension',
            icon: '[ASK]',
            description: 'Интерактивные диалоги с пользователем через TUI. Один вопрос (question) или серия с табами (questionnaire).',
            details: 'Два инструмента для опроса пользователя. question: один вопрос со стрелками ↑↓, Enter для выбора, Esc для отмены, опция "Type something..." для свободного ввода. questionnaire: несколько вопросов с таб-навигацией (Tab/←→), каждая вкладка — отдельный вопрос, финальная вкладка Submit с сводкой. В неинтерактивном режиме — авто-выбор первого варианта.',
            tools: [
            ...
            commands: [],
            hotkeys: ['↑↓ — навигация по вариантам', 'Enter — выбор', 'Esc — отмена', 'Tab/←→ — переключение табов (questionnaire)'],
            usage: 'Расширение для агентов. Используется автоматически, когда агенту нужно уточнить информацию у пользователя. Для ручного вызова: просто ответь на вопрос агента.'
        },
        {
            id: 'fan-soul',
            name: 'fan-soul',
