...
вердикта) → тестирование → коммит. В финале: полная верификация, code review, security-аудит, smoke/e2e-тесты,
...
Фазы 1..N: Для каждой функции в roadmap (этап за этапом)
  ...
  └── (phase-gate) опциональный security-аудит этапа, если он затрагивал auth/секреты/пользовательский ввод

Финальная фаза: Завершение
  ├── Полная верификация фичи
  ├── Code review (G.1.5, code-review worker)
  ├── Security-аудит (G.1.6, security worker)
  ├── Smoke-тестирование
...
- delegate_task — для запуска под-агентов (включая read-only security и code-review для финального аудита)
