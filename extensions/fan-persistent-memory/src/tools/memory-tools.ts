...
export function registerMemoryTools(
    ...
    async execute(_toolCallId, params, _signal, onUpdate, ctx) {
      ...
      try {
        ...
        const memory = repo.create({
          ...
          confidence,
          embedding,
          embeddingModel: embedding ? config.embeddings.model : undefined,
        });
  ...
  pi.registerTool({
    ...
    label: "Search Memory",
    description:
      "Search persistent memory for relevant information. Use to recall operator preferences, project knowledge, past decisions, or instructions.",
    promptSnippet:
    ...
    promptGuidelines: [
      ...
      "Combines keyword (FTS) and semantic (embedding) search for best results.",
    ],
    parameters: Type.Object({
      query: Type.String({ description: "Search query", minLength: 1 }),
      category: Type.Optional(
        Type.String({ description: "Filter by category", enum: [...Categories] })
      ),
      scope: Type.Optional(
        Type.String({
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
      ...
      try {
        // Use full RAG retriever if available
        const retriever = getRetriever?.();
        if (retriever) {
          const results = await retriever.retrieve(params.query, {
            limit,
            scope: (params.scope as MemoryScope | "both") ?? "both",
        ...
        // Fallback: simple keyword search when retriever is not available
        const { global, project } = getRepos();
        const cat = params.category as MemoryCategory | undefined;
        const scope = params.scope as MemoryScope | "both" | undefined;

        const globalResults = global.findActive({ category: cat, limit });
        const projectResults = project?.findActive({ category: cat, limit }) ?? [];
