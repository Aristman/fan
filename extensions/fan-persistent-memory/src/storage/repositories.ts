...
export class MemoryRepository {
  ...
  /** Get underlying database instance (for FTS/vector search). */
  ...
  findAllWithEmbeddings(limit?: number, offset?: number): (MemoryRow & { rowid: number })[] {
    let sql = "SELECT rowid, * FROM memories WHERE archived = 0 AND embedding IS NOT NULL";
    const params: unknown[] = [];
    ...
    return this.db.getDb().prepare(sql).all(...params) as (MemoryRow & { rowid: number })[];
  }
  ...
  cloneTo(otherRepo: MemoryRepository, id: string): Memory | null {
    ...
    return otherRepo.create({
      ...
      tags: mem.tags,
      scope: mem.scope,
      projectPath: mem.projectPath ?? undefined,
      confidence: mem.confidence,
      // Don't transfer raw embedding — it will be re-generated in the target repo
    });
  }
