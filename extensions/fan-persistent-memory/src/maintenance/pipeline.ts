...
import { computePairwiseSimilarity } from "../rag/vector-store.js";
...
export class MaintenancePipeline {
  ...
  async run(): Promise<MaintenanceReport> {
    ...
    const step7 = this.step_archiveCleanup();
    steps.push(step7);
    ...
    const summary = [
      ...
      step7.detail && step7.detail !== "0 deleted" ? `• ${step7.detail}` : null,
    ]
      .filter(Boolean)
      .join("\n  ");
  ...
  private step_clustering(): MaintenanceStep {
    ...
    for (const [category, catMemories] of byCategory) {
      ...
      for (const group of groups) {
        // Clear existing clusters for these memories before creating new ones
        for (const memId of group) {
          this.repo.setClusterId(memId, null);
        }
    ...
    // Clean up orphaned clusters (not referenced by any memory)
    const allClusters = this.clusterRepo.findAll();
    for (const cluster of allClusters) {
      const hasMember = memories.some((m) => m.clusterId === cluster.id);
      if (!hasMember) {
        this.clusterRepo.delete(cluster.id);
      }
    }
    }
  ...
  private step_summarization(): MaintenanceStep {
    ...
    // TODO: Implement when LLM integration is available in extensions
    const clusters = this.clusterRepo.findAll();

    return {
      name: "Summarization",
      status: "skipped",
      detail: `LLM summarization not yet implemented (${clusters.length} clusters pending)`,
    };
  }
