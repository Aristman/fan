...
describe("TC-F-2.1-2: maskSecret — маскирование секретов (4+4 символа)", () => {
	...
	it("строки короче порога (len < 9) возвращаются без изменений", async () => {
		// Порог 9 = 4 видимых символа головы + минимум 2 скрытых символа середины
		// (на границе порога хвост укорачивается до 3); спека §6.1 порог не определяет — контракт фиксирует тест.
		// 8 символов = 4+4 без скрытой середины: показывать целиком опасно, маскировать нечего.
		const { maskSecret } = requireExport(await loadReport(), "maskSecret");
