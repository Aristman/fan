...
- Команда `/voice` и шорткат `F12` (настраивается)
...
2. **Автоматическая установка whisper-cli и ffmpeg** — если инструменты не найдены, wizard покажет платформу, размер бинарника и попросит подтверждение перед скачиванием из FAN Store. Для Windows используется сборка `ffmpeg` с поддержкой DirectShow (gyan.dev full), которая умеет перечислять и записывать микрофоны.
3. **Выбор микрофона** — на Windows wizard перечисляет обнаруженные DirectShow-устройства и предлагает выбрать нужное (выбор сохраняется в `AUDIO_DEVICE`). На Linux/macOS доступен ручной ввод имени (по умолчанию `default`).
...
7. **Горячая клавиша** — по умолчанию `f12`. Все статусы (запись, распознавание, готово) показываются в футере расширения.
...
В TUI нажмите `F12` или введите команду `/voice`:
