...
2. **Проверка аудиоустройства** — обнаружение микрофона.
3. **Выбор языка распознавания** — auto, ru, en, de, fr, es, zh, ja или другой.
4. **Максимальная длительность записи** — от 5 до 300 секунд.
5. **Подключение Ollama** (опционально) — ввод адреса сервера, выбор модели, настройка system prompt.
6. **Горячая клавиша** — по умолчанию `Ctrl+Shift+V`.
7. **Скачивание whisper модели** — ggml-base.bin (~142 МБ).
8. **Сохранение конфигурации** — `.env` в директории расширения.
