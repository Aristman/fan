...
Портирован из fan-orchestrator с сохранением стабильной архитектуры воркеров. Расширение для FAN, добавляющее режим координатора, доску задач, 10 специализированных воркеров, систему разрешений и Pipeline Mode для многофазных проектов.

## Highlights

- **10 специализированных воркеров** — explore, plan, implement, verify, security, bug-fix, code-research, code-review, tests-impl, docs-impl
...
### Slot pool
- **Read-only воркеры** (explore, plan, verify, security, code-review, code-research): до `config.parallelWorkers` (default 3) одновременно
...
| `agents.js` | Реестр агентов: 10 воркеров, координаторский промпт, discovery |
...
| 🔎 code-review | Read-only | Static review of diff/PR/project with verdict system |
